...
- **Встроенные обновления** — в настройках показана версия и кнопка «Проверить обновление»; если есть новее, по клику скачивается `.dmg`, приложение заменяется и перезапускается.
...
1. Скачайте `ClaudeCodexLimits-1.9.dmg` со страницы [Releases](../../releases).
