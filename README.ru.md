...
**Codex** (OpenAI). При открытии панели (или по кнопке «Обновить») приложение тянет
**живые** данные с того же бэкенда, что и Codex CLI — `GET /backend-api/wham/usage` —
авторизуясь локальным токеном из `~/.codex/auth.json` (с авто‑обновлением через токен‑эндпоинт
OpenAI, когда он протух). `primary_window` — 5‑часовое окно, `secondary_window` — 7‑дневное.
Фоновые тики в сеть не ходят — между открытиями берётся самый свежий локальный журнал сессий
...
1. Скачайте `ClaudeCodexLimits-1.4.dmg` со страницы [Releases](../../releases).
